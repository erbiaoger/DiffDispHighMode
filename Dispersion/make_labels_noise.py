...
def get_dshift(nt, dt, nx, dx, nfft, cpr):
    ...
    nl = len(cpr[1][1])
    x1 = np.random.randint(int(nl*0.1), int(nl*0.2))
    x2 = np.random.randint(int(nl*0.4), int(nl*0.5))
    cpr[1][1][:x1] = np.inf
    cpr[1][1][x2:] = np.inf
    ...
    nl = len(cpr[2][1])
    x1 = np.random.randint(int(nl*0.1), int(nl*0.2))
    x2 = np.random.randint(int(nl*0.5), int(nl*0.6))
    cpr[2][1][:x1] = np.inf
    cpr[2][1][x2:] = np.inf
...
def process_combination(nt, dt, nx, dx, nfft, period, fluctuation_percentage, cmin, cmax, fmin, fmax, h, v):
    ...
    for ii in range(3):
        try:
            ...
            # 添加噪声
            # dshift = dshift + np.random.randn(*dshift.shape) * 0.05 * dshift.max()
            f, c, out = fj(dshift, dx, dt, cmin, cmax)
            # f, c, out, U, t = park(dshift, dx, dt, cmin, cmax, dc, fmin, fmax)
            ...
            show_label(f, c, out, cprr, fmin, fmax, ii, fname)
        except Exception as e:
            print(e)
            continue
        ...
        for h2 in np.linspace(0.01, 0.03, 3):
            for h3 in np.linspace(0.01, 0.03, 3):
                for h4 in np.linspace(0.03, 0.07, 3):
                    for v1 in np.linspace(0.25, 0.4, 2):
...
# v1, v2, v3, v4, v5 = 0.1, 0.2, 0.3, 0.8, 1.0
...
# dshifts, fs, vfs = [], [], []
# nl = len(cpr[0][1])
# x1 = np.random.randint(int(nl*0.15), int(nl*0.25))
# x2 = np.random.randint(int(nl*0.25), int(nl*0.4))
# print('nl, x1, x2', nl, x1, x2)
# cpr[0][1][x1:x2] = np.inf
# for imode in range(3):
...
# ax[0].imshow(dshift, aspect='auto', cmap='gray',
#             extent=(t.min(), t.max(),0, 100),origin='lower', interpolation='bilinear')
# ax[1].imshow(out, aspect='auto', cmap='gray',
#             extent=(f.min(), f.max(),c.min(), c.max()),origin='lower')
